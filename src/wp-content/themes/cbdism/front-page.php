...
$custom_posts = get_posts($product_args);
...
<main class="main container index" id="main">
    <div class="row">
        <section class="sec_category col-sm-6 col-xs-12">
            <a class="accordion_menu_btn amb-c text-primary not-scroll" data-toggle="collapse" href="#acd_category"
                ...
                <div class="swiper-wrapper">
                    ...
                    foreach ($parent_terms as $index => $parent_term) :
                        ...
                        <div class="swiper-slide"><a
                                    href="<?php echo get_term_link($parent_term->term_id, $taxonomy); ?>" class='item'>
    ...
    <section class="sec_overall_rank">
        <a class="accordion_menu_btn amb-r text-primary not-scroll" data-toggle="collapse" href="#acd_rank"
           role="button" aria-expanded="false" aria-controls="collapseExample">
    ...
    <script>
        jQuery(function () {
            ...
            var products = jQuery(".slider-ranking .swiper-slide");
            // 総合ランキングのタイトルのソート
            product_titles.sort(function (before, after) {
                var before_rate = jQuery(before).find('.rate').text();
                console.log(before_rate);
                var after_rate = jQuery(after).find('.rate').text();
                if (before_rate < after_rate) return 1;
                else if (before_rate == after_rate) {
                    if (jQuery(before).find('.sum_reviewer').text() > jQuery(after).find('.sum_reviewer').text()) return 1;
                    else return -1;
                } else return -1;
            });
            ...
            products.each(function (index, product) {
                jQuery('.slider-ranking .swiper-wrapper').append(product);
            });
        });
    </script>
</main>
